...
const API_KEY = "79dea625b7ad426e8df101046261205";
...
function render(data) {
  ...
  aqi.textContent =
    data.current.air_quality && data.current.air_quality["us-epa-index"]
...
async function handleSearch() {
  ...
  if (!q) {
    setMessage("Please enter a location", true);
    return;
  }
  setMessage("");
  try {
    ...
    console.error(err);
    setMessage(
      "Could not fetch weather. Check location or your API key and CORS/mixed-content settings.",
      true,
    );
...
function useGeolocation() {
  if (!navigator.geolocation) {
    setMessage("Geolocation not supported by this browser.", true);
    return;
  }
...
function renderHistory() {
  ...
  historyDiv.innerHTML = history
    .map((city) => `<button onclick="quickSearch('${city}')">${city}</button>`)
    .join("");
}
